...
${embeddedFonts}
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @media print {
      ...
      .page { width: 100%; box-shadow: none; padding: 5.5mm 10mm; margin: 0; min-height: auto; }
      @page { size: A4; margin: 0; }
    }
  `;

  html = html.replace(/<style>[\s\S]*?<\/style>/, `<style>${cleanIntegerCss}</style>`);

  fs.writeFileSync('resume.html', html);
  console.log('✓ Applied exact integer typography: 14pt (Sections), 12pt (Orgs), 10pt (Sub-headings), 10pt (Body)');
  ...
  console.log(`Clean Integers (14, 12, 10, 10) => Edu Bottom: ${metrics.eduBottom} / 1123px (${1123 - metrics.eduBottom}px remaining, ${Math.round(metrics.eduBottom/1123*100)}% fill)`);

  await page.screenshot({ path: 'resume_screenshot_integers.png', fullPage: true });
  await browser.close();
})();
