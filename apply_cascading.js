...
(async () => {
  ...
  const rawOriginal = fs.readFileSync('resume.html', 'utf8');
  const embeddedMatch = rawOriginal.match(/@font-face\s*\{[\s\S]*?\}\s*@font-face\s*\{[\s\S]*?\}\s*@font-face\s*\{[\s\S]*?\}\s*@font-face\s*\{[\s\S]*?\}\s*@font-face\s*\{[\s\S]*?\}\s*@font-face\s*\{[\s\S]*?\}/);
  const embeddedFonts = embeddedMatch ? embeddedMatch[0] : '';

  const cascadingHtml = `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
${embeddedFonts}
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Strategic Cascading Typography Hierarchy */
    ...
    @media print {
      ...
      .page { width: 100%; box-shadow: none; padding: 6.5mm 10mm; margin: 0; min-height: auto; }
    ...
    <section>
      ...
      <div class="company-block">
        <div class="job-header">
          ...
          <span class="job-meta">Dec 2024 – Present</span>
        </div>
      ...
      <div class="company-block company-block-sep">
        <div class="job-header">
          ...
          <span class="job-meta">Dec 2022 – Dec 2024, Pune</span>
        </div>
        <ul>
      ...
      <div class="company-block company-block-sep">
        <div class="job-header">
          ...
          <span class="job-meta">Oct 2015 – Apr 2017, Gurgaon</span>
        </div>
        <ul>
...
`;
  ...
  fs.writeFileSync('resume.html', cascadingHtml);
  fs.writeFileSync('cvs/resume.html', cascadingHtml);
  ...
  console.log(`Cascading Hierarchy => Edu Bottom: ${metrics.eduBottom} / 1123px (${1123 - metrics.eduBottom}px margin remaining, ${Math.round(metrics.eduBottom/1123*100)}% fill)`);
  ...
  console.log('✓ Strategic cascading hierarchy applied and PDF recompiled!');
  await browser.close();
})();
