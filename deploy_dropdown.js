...
const cleanDropdownHtml = `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=Lora:ital,wght@0,400;0,600;0,700;1,400&family=Merriweather:ital,wght@0,400;0,700;1,300;1,400&family=Source+Serif+4:ital,opsz,wght@0,8..60,400;0,8..60,600;0,8..60,700;1,8..60,400&display=swap" rel="stylesheet">
  <style>
${embeddedFonts}
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .font-dropdown:hover, .font-dropdown:focus {
      border-color: #3b82f6;
    }
...
<body>
  ...
  <!-- Top Toolbar with 4 Font Dropdown -->
  <div class="top-toolbar">
    <div class="font-select-wrapper">
      <label for="fontSelect">Font:</label>
      <select id="fontSelect" class="font-dropdown" onchange="changeFont(this.value)">
        <option value="garamond" selected>EB Garamond (Current / Recommended)</option>
        <option value="lora">Lora (Contemporary Serif)</option>
        <option value="merriweather">Merriweather (High Contrast)</option>
        <option value="sourceserif">Source Serif 4 (Modern Classical)</option>
      </select>
    </div>

    <div class="divider-v"></div>

    <a href="Vinay_Kumar_CV.pdf" download="Vinay_Kumar_CV.pdf" class="btn">⬇️ PDF</a>
    <a href="Vinay_Kumar_CV.docx" download="Vinay_Kumar_CV.docx" class="btn btn-secondary">⬇️ Word (.docx)</a>
  </div>
  ...
  <script>
    const fontMap = {
      garamond: '"EB Garamond", "Times New Roman", Times, serif',
      lora: '"Lora", Georgia, serif',
      merriweather: '"Merriweather", Georgia, serif',
      sourceserif: '"Source Serif 4", "Times New Roman", serif'
    };

    function changeFont(fontKey) {
      const selectedFont = fontMap[fontKey] || fontMap.garamond;
...
fs.writeFileSync('resume.html', cleanDropdownHtml);
fs.writeFileSync('cvs/resume.html', cleanDropdownHtml);
console.log('✓ Clean 4-font dropdown deployed successfully!');
